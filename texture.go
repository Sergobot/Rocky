...
type Texture struct {
    // OpenGL texture ID
    texture uint32
    // OpenGL texture unit is required 
    // if there are more than one texture
    // are used in a single shader.
    unit uint32
...
func (t *Texture) LoadFromFile(file string) {
    t.imageLoaded = false

    // Load an image from file
    imgFile, err := os.Open(file)
	if err != nil {
		log.Printf("Failed to load texture from %q: %v", file, err)
        return
	}
	...
	if err != nil {
        log.Println("Failed to decode an image:", err)
        return
	}
...
func (t *Texture) Bind() {
    if t.imageLoaded {
        ...
        log.Println("Prevented binding a nonexistent texture")
    }
}
...
func (t *Texture) Unbind() {
    if t.imageLoaded {
        ...
        log.Println("Prevented unbinding a nonexistent texture")
    }
}

// SetTextureUnit sets a unit the texture is using while drawing.
// Required when a few textures are used in a single shader.
func (t *Texture) SetTextureUnit(unit uint32) {
    t.unit = unit
}
